...
const createPool = () => {
  return mysql.createPool({
    ...
    database: process.env.DB_NAME || 'primedrive',
...
const init = async () => {
  try {
    ...
    console.log('Connected to MySQL database: primedrive');
    connection.release();
    ...
    if (users.length === 0) {
      ...
      const [result] = await pool.execute(
        `INSERT INTO users (user_fname, user_lname, user_email, user_password, user_role, user_contact) 
         VALUES (?, ?, ?, ?, ?, ?)`,
        ['Admin', 'User', defaultAdminEmail, hashedPassword, 'admin', '1234567890']
      );
      ...
      console.log('Default admin user created: admin@primedrive.com / admin123');
    }
  } catch (error) {

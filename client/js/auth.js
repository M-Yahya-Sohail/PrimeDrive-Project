...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setupPasswordToggle('regPassword', 'toggleRegPassword', 'toggleRegPasswordIcon');
    setupPasswordToggle('regConfirmPassword', 'toggleRegConfirmPassword', 'toggleRegConfirmPasswordIcon');
        ...
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (response.user.role !== selectedRole) {
                    window.app.showAlert(`This account is registered as a ${response.user.role}, not as ${selectedRole}. Please select the correct role.`, 'danger');
                ...
                const errorMessage = error.message || 'Login failed. Please check your credentials and make sure the server is running.';
        ...
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await window.app.apiRequest('/auth/register', {
                    method: 'POST',
                    body: JSON.stringify({ name, email, password, phone, address, role: selectedRole })
                });
                ...
                window.app.showAlert(error.message || 'Registration failed. Please try again.', 'danger');
                submitBtn.disabled = false;
                submitBtn.innerHTML = originalText;
            }
        });
